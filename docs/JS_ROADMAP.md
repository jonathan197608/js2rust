...
### 1.3-bis P2 补充 — 不确定项核实（15 项）

> 来源：2026-06-24 JS_FEATURE_EVALUATION.md 文档审计，标记 ⚠️ 的不确定项需逐项深入核实代码/编写测试用例确认。

| # | 类别 | 任务 | 说明 | 复杂度 | 状态 |
|---|------|------|------|--------|------|
| 1 | 类型推断 | `typeof` 运算符 | 验证 `@typeName(@TypeOf(x))` 输出是否与 JS `typeof` 行为一致 | 低 | 📋 待开始 |
| 2 | 类型推断 | `null` 字面量 | 验证 `NullLiteral` 返回 `None` 类型是否导致推断错误 | 低 | 📋 待开始 |
| 3 | 类型推断 | `undefined` → `null` | 验证转换后运行时行为（Zig `null` vs JS `undefined`） | 低 | 📋 待开始 |
...
| 8 | 语句 | `for-in` 静态 struct | 验证静态 struct 展开循环是否正确处理所有字段类型（忽略方法） | 低 | 📋 待开始 |
| 9 | 语句 | 标签语句 | 编写测试验证 `break label` / `continue label` 行为 | 低 | 📋 待开始 |
| 10 | 代码生成 | 可选链 `?.` | 验证生成代码 `if (obj) |v| v.prop else null` 是否存在空指针解引用 | 中 | 📋 待开始 |
| 11 | 代码生成 | 闭包可变捕获 | 验证 `self.x.*` 解引用是否触发 Zig 借用检查器错误 | 高 | 📋 待开始 |
| 12 | 代码生成 | 模板字符串 `allocPrint` | 验证内存是否正确释放（arena reset 时机） | 中 | 📋 待开始 |
| 13 | 边缘情况 | 大型字面量 | 创建 1000+ 元素数组/对象测试是否触发编译器栈溢出 | 低 | 📋 待开始 |
| 14 | 边缘情况 | 深层嵌套调用 | 创建 `a(b(c(d(e(f())))))` 测试编译行为 | 低 | 📋 待开始 |
| 15 | 边缘情况 | Unicode 标识符 | 验证中文变量名是否被 oxc 解析器和代码生成正确处理 | 低 | 📋 待开始 |
...
4. **P2: 不确定项核实** — 完成 1.3-bis 中的 15 项核实任务（优先低复杂度项：#1, #2, #3, #8, #9, #13, #14, #15）
