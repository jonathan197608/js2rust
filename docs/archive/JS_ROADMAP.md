...
| 任务 | 说明 | 理由 | 状态 | 优先级 |
|------|------|------|------|--------|
| 解构默认值 | 支持 ES6 完整语法 | 语言完整性 | 📋 待开始 | 中 |
| 多 spread 合并 `{ ...a, ...b }` | 支持 ES6 完整语法 | 语言完整性 | 📋 待开始 | 中 |
| 正则表达式引擎 | 引入 C 库（如 `pcre2`） | 实用性 | 📋 待开始 | 中 |
| 未覆盖特性测试 | `instanceof`/`in`/`Date`/`Object` 方法等 | 测试覆盖率 | 📋 待开始 | 中 |
| Class 字段类型推断 | 根据构造函数推断字段类型 | 类型安全 | 📋 待开始 | 中 |
| 嵌套函数声明 | 自动提升到模块顶层 | 语言完整性 | 📋 待开始 | 中 |
| `instanceof` / `in` 运算符测试 | 已实现但未测试 | 测试覆盖率 | 📋 待开始 | 中 |
| `Date` 方法测试 | 已实现但未测试 | 测试覆盖率 | 📋 待开始 | 中 |
| `Object` 方法测试 | 已实现但未测试 | 测试覆盖率 | 📋 待开始 | 中 |
| 标签语句测试 | 已实现但未测试 | 测试覆盖率 | 📋 待开始 | 低 |
| `for-in` 静态 struct 集成 | 已实现但 showcase-project 未集成 | 集成验证 | 📋 待开始 | 中 |
...
1. **P2: 提高测试覆盖率** — 未覆盖特性 (`instanceof`/`in`/`Date`/`Object`)
2. **P2: 正则表达式引擎** — 引入 pcre2 或实现迷你引擎
3. **P2: 解构默认值** — 支持 ES6 完整语法
