...
// ── Tests ──
// The stub host_regex_* implementations now support simple literal patterns
// (no metacharacters) via substring matching, so these tests can run under
// both stubs and real regex engines.
