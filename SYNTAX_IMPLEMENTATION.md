...
## 12. 生成代码的 init/deinit 生命周期

### 12.1 两层结构

| 层级 | 位置 | 生成者 | 职责 |
|------|------|--------|------|
| **模块级** | `<module>.zig` | `codegen/mod.rs` | 管理本模块的全局 HashMap（动态属性对象） |
| **编排级** | `lib.zig` | `project.rs` | 聚合所有模块，对外暴露统一入口 |

### 12.2 init_js2rust()

- **始终生成**（每个模块 + lib.zig）
- 模块级：调用 `js_allocator.setGlobalAllocator(allocator)` + 初始化本模块的 HashMap
- lib.zig：调用自身的 `setGlobalAllocator` + 委托到每个子模块的 `init_js2rust`
- 测试代码：`init_js2rust(allocator)` 在每个 test 块开头调用

### 12.3 deinit_js2rust()

- **条件生成**：仅当模块存在需要释放的 HashMap 时才生成（`codegen/mod.rs`）
- lib.zig：始终生成空壳函数，仅委托到有 `deinit_js2rust` 的子模块
- 测试代码：`defer deinit_js2rust()` 在每个 test 块中（零开销，Zig 优化掉空函数调用）
- 判断逻辑：扫描 `init_globals_code` 中的 `.init()` 调用提取 HashMap 变量名

### 12.4 C ABI 层

当 `include_windows_stub = true` 时，lib.zig 额外生成 C ABI 导出：
- `export fn js2rust_init()` → 使用 `page_allocator` 调用 `init_js2rust`
- `export fn js2rust_deinit()` → 调用 `deinit_js2rust`
...
| `for await...of` | ❌ | `@compileError`（需要异步运行时） |
