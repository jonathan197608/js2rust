...
pub fn generate(opts: &ProjectOptions) -> Result<(), String> {
    ...
    for module in &opts.per_file_code {
        let has_host = !opts.host_header.is_empty() || !opts.async_host_fn_names.is_empty();
        let mod_zig = generate_module_zig(
            module,
            &opts.async_host_fn_names,
            opts.needs_regex,
            has_host,
        );
...
///
/// When `has_host` is true, also injects:
/// - `const host = @import("host.zig");` (always available, not conditional on `host.` usage)
/// - `const assert = struct { ... }` — a global assert object with `sameValue` / `notSameValue`
///   methods, so test262 tests can call `assert.sameValue(a, b, msg)` without importing runtime.js.
fn push_runtime_imports(out: &mut String, needs_regex: bool, has_host: bool) {
    ...
    // Host functions + assert global object
    // Injected unconditionally (when has_host) so that `assert.sameValue(...)`
    // works in every file without importing runtime.js.
    if has_host {
        out.push_str("// Host functions (Rust via C ABI)\n");
        out.push_str("const host = @import(\"host.zig\");\n");
        out.push('\n');
        // Dummy error types so `assert.throws(TypeError, ...)` compiles.
        // The type identity is not used at runtime — Zig panics abort the
        // process, so `throws` just verifies the closure panics.
        out.push_str("const TypeError = struct {};\n");
        out.push_str("const Test262Error = struct {};\n");
        out.push('\n');
        out.push_str("// test262 assert global object — available without import\n");
        out.push_str("const assert = struct {\n");
        out.push_str(
            "    fn sameValue(actual: anytype, expected: anytype, message: []const u8) void {\n",
        );
        out.push_str("        if (actual != expected) { _ = host.assert_fail(message); }\n");
        out.push_str("    }\n");
        out.push_str(
            "    fn notSameValue(actual: anytype, expected: anytype, message: []const u8) void {\n",
        );
        out.push_str("        if (actual == expected) { _ = host.assert_fail(message); }\n");
        out.push_str("    }\n");
        // assert.throws(ExpectedError, closure, message)
        // Calls closure.call(); if it panics (throw), the process aborts
        // (subprocess isolation in the test runner catches this). If it
        // returns normally, the expected throw didn't happen → test fails.
        //
        // Two closure shapes are generated by the transpiler:
        //   1. No-capture: `_fn_expr_0` is a *type*, `call()` is a namespace fn
        //   2. Captured:   `_fn_expr_0{ .s = s }` is an *instance*,
        //                  `call(self: *@This())` needs a mutable pointer.
        // For case 2, copy to a `var` so `&c` is `*T` not `*const T`.
        out.push_str(
            "    fn throws(comptime ExpectedError: type, closure: anytype, message: []const u8) void {\n",
        );
        out.push_str("        _ = ExpectedError;\n");
        out.push_str("        if (@TypeOf(closure) == type) {\n");
        out.push_str("            closure.call();\n");
        out.push_str("        } else {\n");
        out.push_str("            var c = closure;\n");
        out.push_str("            c.call();\n");
        out.push_str("        }\n");
        out.push_str("        _ = host.assert_fail(message);\n");
        out.push_str("    }\n");
        out.push_str("};\n");
        out.push('\n');
    }
...
fn generate_module_zig(
    ...
    push_runtime_imports(&mut out, needs_regex, has_host);

    // Host regex import (conditional — only when the module uses regex host functions)
    ...
    if !async_host_fn_names.is_empty() {
...
fn generate_orchestrator_lib(opts: &ProjectOptions) -> String {
    ...
    let has_host = !opts.host_header.is_empty() || !opts.async_host_fn_names.is_empty();
    push_runtime_imports(&mut out, opts.needs_regex, has_host);

    // Host regex import (conditional)
    if opts.needs_regex {
        out.push_str("// Host regex functions (Rust via C ABI)\n");
        out.push_str("const host_regex = @import(\"host_regex.zig\");\n");
        out.push('\n');
